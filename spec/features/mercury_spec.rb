describe "Mercury", js: true do
  let!(:product) { FactoryGirl.create(:product, name: 'iPad') }

  before do
    @gateway = Spree::Gateway::MercuryExpress.create!({
  ...
  def fill_in_billing
    ...
    # City, State and ZIP must all match for Mercury to be happy
  ...
  xit "pays for an order successfully" do
    ...
    click_button "Save and Continue"
    find("#mercury_button").click
    switch_to_mercury_login
    login_to_mercury
  ...
  context "with 'Sole' solution type" do
    ...
    xit "passes user details to Mercury" do
      add_to_cart(product)
      click_button 'Checkout'
      within("#guest_checkout") do
        fill_in "Email", with: "test@example.com"
  ...
  xit "includes adjustments in Mercury summary" do
    add_to_cart(product)
  ...
  context "line item adjustments" do
    ...
    xit "includes line item adjustments in Mercury summary" do
      add_to_cart(product)
      # TODO: Is there a better way to find this current order?
      order = Spree::Order.last
  ...
  context "cannot process a payment with invalid gateway details" do
    ...
    specify do
      ...
      click_button "Save and Continue"
      find("#mercury_button").click
      page.should have_content("Mercury failed. Security header is not valid")
    end
  end
  ...
  context "can process an order with Tax included prices" do
    ...
    context "on countries where the Tax is applied" do
      ...
      xit do
        ...
        within_transaction_cart do
          # included taxes should not go on mercury
  ...
  context "as an admin" do
    ...
    context "refunding payments" do
      ...
      xit "can refund payments fully" do
        click_button "Refund"
        page.should have_content("Mercury refund successful")

        payment = Spree::Payment.last
        mercury_checkout = payment.source.source
        mercury_checkout.refund_transaction_id.should_not be_blank
        mercury_checkout.refunded_at.should_not be_blank
        mercury_checkout.state.should eql("refunded")
        mercury_checkout.refund_type.should eql("Full")
      ...
      xit "errors when given an invalid refund amount" do
        ...
        click_button "Refund"
        page.should have_content("Mercury refund unsuccessful (The partial refund amount is not valid)")
      end
    end
  end
end
