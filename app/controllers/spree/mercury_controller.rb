module Spree
  class MercuryController < StoreController
    ...
    def express
      ...
      # Because Mercury doesn't accept $0 items at all.
      # See #10
      # https://cms.mercury.com/uk/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_ECCustomizing
      ...
      begin
        ...
        if pp_response.success?
          ...
          flash[:error] = Spree.t('flash.generic_error', :scope => 'mercury', :reasons => pp_response.errors.map(&:long_message).join(" "))
          redirect_to checkout_state_path(:payment)
        end
      rescue SocketError
        flash[:error] = Spree.t('flash.connection_failed', :scope => 'mercury')
        redirect_to checkout_state_path(:payment)
      end
    end
    ...
    def cancel
      flash[:notice] = Spree.t('flash.cancel', :scope => 'mercury')
      order = current_order || raise(ActiveRecord::RecordNotFound)
      redirect_to checkout_state_path(order.state, mercury_cancel_token: params[:token])
    end
    ...
    def express_checkout_request_details order, items
      { :SetExpressCheckoutRequestDetails => {
          ...
          :ReturnURL => confirm_mercury_url(:payment_method_id => params[:payment_method_id], :utm_nooverride => 1),
          :CancelURL =>  cancel_mercury_url,
    ...
    def payment_details items
      ...
      if item_sum.zero?
        # Mercury does not support no items or a zero dollar ItemTotal
